    ...
    def process(self, model, image_embeds, shift, steps, cfg, seed, scheduler, riflex_freq_index, text_embeds=None,
        ...
        if isinstance(scheduler, dict):
            ...
            sample_scheduler, timesteps,_,_ = get_scheduler(
                scheduler, steps, start_step, end_step, shift, device, transformer.dim, flowedit_args,
                denoise_strength, sigmas=sigmas, log_timesteps=False
            )
        else:
                ...
                for idx, t in enumerate(tqdm(timesteps, disable=multitalk_sampling or wananimate_loop)):
                    ...
                    #region multitalk
                        ...
                        while True: # start video generation iteratively
                            ...
                            force_exact_frames_out = image_embeds.get("force_exact_frames", False)
                            if force_exact_frames_out:
                                req_frames = image_embeds.get("requested_num_frames", num_frames)
                                t = videos.shape[1]
                                if t > req_frames:
                                    videos = videos[:, :req_frames]
                                elif t < req_frames:
                                    pad = videos[:, -1:].repeat(1, req_frames - t, 1, 1)
