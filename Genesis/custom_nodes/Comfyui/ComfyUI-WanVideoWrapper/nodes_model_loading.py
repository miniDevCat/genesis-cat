...
class WanVideoLoraSelectMulti:
        ...
        return {
            "required": {
               "lora_0": (lora_files, {"default": "none"}),
                ...
                "lora_5": (lora_files, {"default": "none"}),
                "strength_5": ("FLOAT", {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.0001, "tooltip": "LORA strength, set to 0.0 to unmerge the LORA"}),
                "lora_6": (lora_files, {"default": "none"}),
                "strength_6": ("FLOAT", {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.0001, "tooltip": "LORA strength, set to 0.0 to unmerge the LORA"}),
                "lora_7": (lora_files, {"default": "none"}),
                "strength_7": ("FLOAT", {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.0001, "tooltip": "LORA strength, set to 0.0 to unmerge the LORA"}),
                "lora_8": (lora_files, {"default": "none"}),
                "strength_8": ("FLOAT", {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.0001, "tooltip": "LORA strength, set to 0.0 to unmerge the LORA"}),
                "lora_9": (lora_files, {"default": "none"}),
                "strength_9": ("FLOAT", {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.0001, "tooltip": "LORA strength, set to 0.0 to unmerge the LORA"}),
            },
            "optional": {
                "prev_lora":("WANVIDLORA", {"default": None, "tooltip": "For loading multiple LoRAs"}),
                "blocks":("SELECTEDBLOCKS", ),
    ...
    FUNCTION = "getlorapath"
    CATEGORY = "WanVideoWrapper"
    DESCRIPTION = "Select up to 10 LoRA models from ComfyUI/models/loras"

    def getlorapath(self, lora_0, strength_0, lora_1, strength_1, lora_2, strength_2, 
                lora_3, strength_3, lora_4, strength_4, lora_5, strength_5,
                lora_6, strength_6, lora_7, strength_7, lora_8, strength_8,
                lora_9, strength_9, blocks={}, prev_lora=None, 
...
def add_lora_weights(patcher, lora, base_dtype, merge_loras=False):
    ...
    for l in lora:
        ...
        lora_sd = standardize_lora_key_format(lora_sd)
        log.info(f"LoRA has {len(lora_sd)} keys after standardization")

        if l["blocks"]:
            lora_sd = filter_state_dict_by_blocks(lora_sd, l["blocks"], l.get("layer_filter", []))
            log.info(f"LoRA has {len(lora_sd)} keys after block filtering")
        ...
        # normal LoRA patch
            ...
            log.info(f"Applying LoRA with {len(lora_sd)} keys to model...")
            # 使用本地的 load_lora_for_models_mod 而不是 comfy.sd 的版本
            patcher = load_lora_for_models_mod(patcher, lora_sd, lora_strength)
            log.info(f"LoRA application completed")
...
class WanVideoModelLoader:
        ...
        return {
            ...
            "optional": {
                "attention_mode": ([
                    ...
                    ], {"default": "sdpa", "tooltip": "Attention implementation. sageattn_3_fp4 uses FP4 microscaling for RTX 5090 (1038 TOPS), sageattn_3 uses INT8/FP16, sageattn_3_fp8 uses FP8 for maximum speed"}),
    ...
    def loadmodel(self, model, base_precision, load_device,  quantization,
        ...
        # Check if model requires scaled quantization
        is_scaled_model = "scaled_fp8" in sd
        is_scaled_quantization = "scaled" in quantization or quantization in ["fp4_scaled", "fp4_scaled_fast"]

        if is_scaled_model and not is_scaled_quantization:
        ...
        # PATCH: Auto-detect multitalk_audio_proj in merged models (merged safetensors)
        if sd is not None and any(k.startswith("multitalk_audio_proj.") for k in sd.keys()) and not hasattr(transformer, "multitalk_audio_proj"):
            log.info("Detected multitalk_audio_proj in main model (merged model), auto-loading...")

            # Create the audio projection module
            from .multitalk.multitalk import AudioProjModel, SingleStreamMultiAttention
            from .wanvideo.modules.model import WanLayerNorm

            # Initialize AudioProjModel with InfiniteTalk defaults matching weights
            ...
            # Initialize audio_cross_attn for each block
            for block in transformer.blocks:
                with init_empty_weights():
            ...
            log.info("Auto-initialized infinitetalk from merged model weights")
        transformer.eval()
        ...
        if "fp8_e4m3fn" in quantization or "fp4" in quantization:
            # FP4 uses FP8 for weight storage, FP4 is only for attention
            weight_dtype = torch.float8_e4m3fn
            if "fp4" in quantization:
                log.info("FP4 mode: Using FP8 (e4m3fn) for weight storage, FP4 for attention computation")
        ...
        # PATCH: Re-attach multitalk module after linear replacement and re-load its weights
        if sd is not None and any(k.startswith("multitalk_audio_proj.") for k in sd.keys()) and not hasattr(transformer, "multitalk_audio_proj"):
            log.info("Re-attaching multitalk module after linear replacement")
            from .multitalk.multitalk import AudioProjModel, SingleStreamMultiAttention
            from .wanvideo.modules.model import WanLayerNorm
            ...
            # Initialize audio_cross_attn for each block if needed
            if not hasattr(transformer.blocks[0], "audio_cross_attn"):
                _num_heads = getattr(transformer, "num_heads", 40)
                _dim = getattr(transformer, "dim", 5120)
                for block in transformer.blocks:
                    block.norm_x = WanLayerNorm(_dim, transformer.eps, elementwise_affine=True).to(device=device, dtype=base_dtype)
                    block.audio_cross_attn = SingleStreamMultiAttention(
                        dim=_dim,
                        encoder_hidden_states_dim=768,
            ...
            # Assign weights for multitalk module
            try:
                for name, value in sd.items():
                    if name.startswith("multitalk_audio_proj."):
                        set_module_tensor_to_device(transformer, name.replace("_orig_mod.", ""), device=transformer_load_device, dtype=base_dtype, value=value)
                log.info("Loaded multitalk weights after re-attachment")
            except Exception as e:
                log.warn(f"Failed to load multitalk weights after re-attach: {e}")
                transformer.patched_linear = True

        if "fast" in quantization or quantization == "fp4_scaled_fast":
            if lora is not None and not merge_loras:
                raise NotImplementedError("fast quantization modes are not supported with unmerged LoRAs")
            from .fp8_optimization import convert_fp8_linear
            if "fp4" in quantization:
                log.info("FP4 fast mode: Using FP8 weights with fast matmul + FP4 attention")
                log.info("Make sure to select 'sageattn_3_fp4' in attention_mode for full FP4 acceleration")
            ...
            if "scaled" in quantization:
                log.info("FP4 scaled mode: Using scaled FP8 weights + FP4 attention")
                convert_fp4_linear(transformer, base_dtype, params_to_keep, scale_weight_keys=scale_weights)
            else:
                log.info("FP4 experimental mode: Using FP8 weights + FP4 attention")
        ...
        patcher.model["fp8_matmul"] = "fast" in quantization or quantization in ["fp4_scaled_fast"]
