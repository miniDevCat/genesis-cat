...
def patch_weight_to_device(self, key, device_to=None, inplace_update=False, backup_keys=False, scale_weight=None):
    ...
    print(f"[LoRA] Applying {patch_count} patch(es) to key: {key}")

    weight, set_func, convert_func = get_key_weight(self.model, key)
    inplace_update = self.weight_inplace_update or inplace_update
    
    # 记录原始权重范数
    original_weight_norm = torch.norm(weight.float()).item()
    ...
    # 记录修改后的权重范数
    new_weight_norm = torch.norm(out_weight.float()).item()
    weight_change = abs(new_weight_norm - original_weight_norm) / (original_weight_norm + 1e-8) * 100
    print(f"[LoRA] Weight norm changed from {original_weight_norm:.4f} to {new_weight_norm:.4f} ({weight_change:.2f}% change)")
...
def apply_lora(model, device_to, transformer_load_device, params_to_keep=None, dtype=None, 
        ...
        print("[DEBUG] apply_lora called - binding patch_weight_to_device method")
        print(f"[DEBUG] model.patches has {len(model.patches)} keys")
        if len(model.patches) > 0:
            print(f"[DEBUG] First 10 patch keys: {list(model.patches.keys())[:10]}")
        model.patch_weight_to_device = types.MethodType(patch_weight_to_device, model)
        print(f"[DEBUG] patch_weight_to_device bound: {hasattr(model, 'patch_weight_to_device')}")
        
        # 添加计数器来跟踪实际调用次数
        model._patch_call_count = 0
        to_load = []
        ...
        for x in tqdm(to_load, desc="Loading model and applying LoRA weights:", leave=True):
            ...
            for param in params:
                ...
                if key in model.patches:
                    print(f"[DEBUG] Found patch for key: {key}")
                    model._patch_call_count += 1
        ...
        # 总结 LoRA 应用情况
        print(f"[DEBUG] LoRA application summary:")
        print(f"  - Total patches in model.patches: {len(model.patches)}")
        print(f"  - Patches actually applied: {getattr(model, '_patch_call_count', 0)}")
