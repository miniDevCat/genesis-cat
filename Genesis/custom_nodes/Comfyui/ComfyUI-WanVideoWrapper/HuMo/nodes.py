...
class HuMoEmbeds:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            ...
            "force_exact_frames": ("BOOLEAN", {"default": False, "tooltip": "Force exact frame count (may affect quality due to model constraints)"})
        },
            "optional" : {
    ...
    def process(self, num_frames, width, height, audio_scale, audio_cfg_scale, audio_start_percent, audio_end_percent, force_exact_frames=False, whisper_model=None, vae=None, reference_images=None, audio=None, tiled_vae=False):
        ...
        original_frame_num = pixel_frame_num
        
        if not force_exact_frames:
            # Apply HuMo model constraint: frames must be 4n+1
            pixel_frame_num = 4 * ((pixel_frame_num - 1) // 4) + 1
            if original_frame_num != pixel_frame_num:
                log.info(f"HuMo adjusted frame count from {original_frame_num} to {pixel_frame_num} (4n+1 constraint)")
        else:
            log.info(f"HuMo forcing exact frame count: {pixel_frame_num} (may affect quality)")
            
        if force_exact_frames:
            latent_frame_num = math.ceil((pixel_frame_num + 3) / 4)
        else:
        ...
        embeds = {
            ...
            "requested_num_frames": original_frame_num,
            "force_exact_frames": force_exact_frames,
